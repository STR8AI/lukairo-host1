...
/**
 * Creates the orbiting platforms container and icons
 */
function createOrbitingPlatforms() {
  ...
  orbitContainer.setAttribute('aria-label', 'Orbiting platform icons');
  orbitContainer.setAttribute('role', 'presentation');

  // Create orbiting icons for each platform
  platformIcons.forEach((platform, index) => {
    ...
    const x = Math.cos(angleRad) * platform.orbitRadius;
    const y = Math.sin(angleRad) * platform.orbitRadius;
    icon.style.left = `${x}px`;
    icon.style.top = `${y}px`;
    icon.style.transform = 'translate(-50%, -50%)';
    
    orbitRing.appendChild(icon);
    orbitContainer.appendChild(orbitRing);
  });
...
globe = new THREE.Mesh(geometry, material);
  ...
  */
  ...
  // Create orbiting platform icons
  createOrbitingPlatforms();
