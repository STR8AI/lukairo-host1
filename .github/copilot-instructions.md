...
The project uses Gulp 4 for build automation:

### Available Gulp Tasks

- `gulp` or `gulp build` - Default build task (runs html, css, js, images tasks)
- `gulp images` - Optimize images using gulp-imagemin
- `gulp html` - Copy HTML files to dist
- `gulp css` - Copy CSS files to dist
- `gulp js` - Copy JavaScript files to dist
- `gulp serve` - Start development server with LiveReload
- `gulp watch` - Watch files for changes and rebuild
- `gulp dev` - Build + start server + watch for changes

### Build Dependencies

All Gulp plugins required by gulpfile.js must be listed in package.json devDependencies:
- `gulp` - Task runner
- `gulp-imagemin` - Image optimization
- `gulp-webserver` - Development server with LiveReload
